# Asistente RAG para NutriFit Chile

Este repositorio tiene el código del proyecto para la Evaluación Parcial N°1 de la asignatura de Soluciones con IA[cite: 7]. Armamos un asistente inteligente conectado a una base de datos local y documentos de la empresa para responder consultas de inventario y políticas al instante[cite: 7].

## Contenido del proyecto
* **data/**: Archivos con el inventario en CSV y las políticas en PDF.
* **app.py**: El script principal que levanta el agente y procesa las preguntas.
* **requirements.txt**: Las librerías necesarias de Python.

## Cómo correrlo en tu equipo

1. **Clona este repositorio:**
   ```bash
   git clone [https://github.com/matycodee/inteligencia.git](https://github.com/matycodee/inteligencia.git)
   cd inteligencia
